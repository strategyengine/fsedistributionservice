...
public class AirdropVerificationServiceImpl implements AirdropVerificationService {
	...
	@Transactional
	@Override
	public PaymentRequestEnt verifyDropComplete(PaymentRequestEnt paymentRequestInitial, List<FseTransaction> transactions, boolean releaseLock) {

		try {

			if (cancelDropRequestRepo
					.exists(Example.of(CancelDropRequestEnt.builder().dropRequestId(paymentRequestInitial.getId()).build()))) {
				try {
			...
			if (MIN_DROP_REQUEST_ID_TO_COLLECT_FEES < paymentRequest.getId() && (paymentRequest.getFeesPaid() == null || Double.valueOf(paymentRequest.getFeesPaid()) == 0)) {
			...
			if (!recipientsToValidate.isEmpty()) {
				...
				List<TransactionEnt> transactionHashes = transactionRepo.findAll(Example.of(TransactionEnt.builder().dropRequestId(paymentRequest.getId()).build()));
				
				log.info("Found transaction hashes to validate size - " + transactionHashes.size());
				shouldHaveRecievedDropButDidNot.addAll(recipientsToValidate.stream()
						.filter(t -> !didReceiveDrop(t, airdDropByToAddress, 
								transactionHashes.stream().filter(p -> p.getDropRecipientId()!=null && p.getDropRecipientId().equals(t.getId())).collect(Collectors.toList())))
			...
			log.info("Finished Airdrop verification for UUID: {} paymentRequest ID: {}", finalizedFeeCollected.getLockUuid(),
					finalizedFeeCollected.getId());
			...
			Optional<FseTrustLine> trustLineForFromAddress = xrplService.getTrustLines(finalizedFeeCollected.getFromClassicAddress(), FseSort.OLD).stream()
			.filter(t -> t.getCurrency().equals(finalizedFeeCollected.getCurrencyName())||t.getCurrency().equals(finalizedFeeCollected.getCurrencyNameForProcess())).findAny();
			
			if(trustLineForFromAddress.isPresent()) {
				if(new BigDecimal(trustLineForFromAddress.get().getBalance()).compareTo(new BigDecimal(finalizedFeeCollected.getAmount()))==-1) {
	...
	public Map<String, List<FseTransaction>> getTransactionMap(PaymentRequestEnt paymentRequest,
		...
		for (FseTransaction t : transactions) {
			if (t.getAmount() != null
					&& (paymentRequest.getTrustlineIssuerClassicAddress().equals(t.getIssuerAddress())|| 
	...
	@Override
	public boolean didReceiveDrop(DropRecipientEnt recipient, Map<String, List<FseTransaction>> airdropByToAddress, List<TransactionEnt> transactionHashes) {
		...
		if(!paymentFound && !transactionHashes.isEmpty()) {
			//TODO move up the fetching of transactionHashes for drop request id after it is being populated for all drops
			//List<TransactionEnt> transactionHashes = transactionRepo.findAll(Example.of(TransactionEnt.builder().dropRecipientId(recipient.getId()).build()));
			List<TransactionResult<Payment>> payments = transactionService.getXrplTransactionsByHashes(transactionHashes);
	...
	protected PaymentRequestEnt collectFees(PaymentRequestEnt p, int totalVerifiedRecipients) {
		...
		if (totalFeesCollected.stripTrailingZeros().equals(BigDecimal.ZERO)
			...
			double serviceFee = isVerifiedDropType(p.getDropType()) ? configService.getDouble(DEFAULT_SERVICE_FEE_PER_INTERVAL_VERFIED) : 
		...
		//	BigDecimal feePortionBurn = remainingToCollect.divide(BigDecimal.valueOf(2l)); //half to Ardy, half to burn
		//	BigDecimal feePortionArdy = remainingToCollect.divide(BigDecimal.valueOf(2l)); //half to Ardy, half to burn
			
			BigDecimal feePortionBurn = remainingToCollect.multiply(new BigDecimal(".9")); //90% burn
			BigDecimal feePortionArdy = remainingToCollect.multiply(new BigDecimal(".1")); //10% Ardy
						
			log.info(String.format("dropid:%s feePortionBurn:%s feePortionArdy:%s", p.getId(), feePortionBurn, feePortionArdy));
			...
			paymentService.allowPaymentNoPersist(
					FsePaymentRequest.builder().trustlineIssuerClassicAddress(FSE_ISSUING_ADDRESS).currencyName("FSE")
							.amount(String.valueOf(feePortionArdy.stripTrailingZeros())).fromClassicAddress(p.getFromClassicAddress())
					...
					SERVICE_FEE_ADDRESS.toBuilder().id(0l).dropRequestId(p.getId()).createDate(now()).payAmount(String.valueOf(feePortionArdy.stripTrailingZeros())).updateDate(now())
							.build());
			
			paymentService.allowPaymentNoPersist(
					FsePaymentRequest.builder().trustlineIssuerClassicAddress(FSE_ISSUING_ADDRESS).currencyName("FSE")
							.amount(String.valueOf(feePortionBurn.stripTrailingZeros())).fromClassicAddress(p.getFromClassicAddress())
							.fromPrivateKey(p.getFromPrivateKey()).fromSigningPublicKey(p.getFromSigningPublicKey())
							.maxXrpFeePerTransaction(p.getMaxXrpFeePerTransaction()).build(),
					BURN_ISSUER_FSE_ADDRESS.toBuilder().id(0l).dropRequestId(p.getId()).createDate(now()).payAmount(String.valueOf(feePortionBurn.stripTrailingZeros())).updateDate(now())
							.build());	
	...
	protected BigDecimal getTotalFeeRequiredForSize(int size, DropType dropType, boolean sendingDifferentIssuer) {
		...
		double serviceFee = isVerifiedDropType(dropType) ? configService.getDouble(DEFAULT_SERVICE_FEE_PER_INTERVAL_VERFIED) : configService.getDouble(DEFAULT_SERVICE_FEE_PER_INTERVAL_UNVERFIED);
